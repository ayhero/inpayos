...
func (c *EmailConfig) Validate() {
	...
	case "account":
		if len(c.Accounts) == 0 {
			panic("email accounts is required when service_name is account")
		}
	...
	case "aliyun":
		if c.Aliyun == nil {
			panic("aliyun config is required when service_name is aliyun")
		}
		c.Aliyun.Validate()
	case "tencent":
		if c.Tencent == nil {
			panic("tencent config is required when service_name is tencent")
		}
		c.Tencent.Validate()
	}
}
...
func (c *EmailAccountConfig) Validate() {
	if c.Name == "" {
		panic("email account name is required")
	}
	if c.Host == "" {
		panic("email account host is required")
	}
	if c.Port <= 0 {
		panic("email account port must be greater than 0")
	}
	if c.Username == "" {
		panic("email account username is required")
	}
	if c.Password == "" {
		panic("email account password is required")
	}
	if c.From == "" {
		panic("email account from is required")
	}
}
